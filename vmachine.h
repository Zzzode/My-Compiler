...
extern long long *text, // 用于存放代码（指令）
    *old_text,    // for dump text segment
    *stack;       // 处理函数调用相关的数据，如调用unordered_map<int, ID>帧（calling frame）或是局部变量等。

extern char *data; // 用于存放初始化了的数据，如int i = 10;

extern long long int *PC, *BP, *SP;
extern long long AX, cycle;      // 寄存器
extern int *current_Id, *symbols;         // ID和符号表
extern int *ID_MAIN_addr;                      // main函数
